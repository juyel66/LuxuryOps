"use client";

import Image from "next/image";

// 👉 replace with your icons
import icon1 from "../../../../public/images/Icon (19).svg";
import icon2 from "../../../../public/images/Icon (20).svg";
import icon3 from "../../../../public/images/Icon (22).svg";
import dividerMobile from "../../../../public/images/Container (23).svg";

const features = [
  {
    title: "Licensed Professionals",
    desc: "Fully vetted and certified security personnel",
    icon: icon1,
  },
  {
    title: "Discreet Service",
    desc: "Subtle protection that respects privacy",
    icon: icon2,
  },
  {
    title: "24/7 Coordination",
    desc: "Round-the-clock protection services",
    icon: icon3,
  },
];

export default function SecurityIntegrated() {
  return (
    <div className="text-white bg-black font-cardo">
      <section className="container px-4 py-20 mx-auto text-center md:py-28">
        
        {/* TOP LABEL (04 with lines both sides) */}
        <div className="items-center justify-center hidden gap-4 mb-6 md:flex">
          <span className="text-[#D4AF37] text-lg">04</span>
            <Image src={dividerMobile} alt="divider mobile" className="block md:hidden" />
            <Image src={dividerMobile} alt="divider" className="hidden md:block" />
        </div>
        <div className="flex items-center justify-start gap-4 mb-6 md:hidden">
          <span className="text-[#D4AF37] text-lg">04</span>
            <Image src={dividerMobile} alt="divider mobile" className="block md:hidden" />
            <Image src={dividerMobile} alt="divider" className="hidden md:block" />
        </div>

    

        {/* HEADING */}
        <h2 className="flex max-w-3xl mx-auto mb-6 text-3xl leading-tight md:hidden md:text-5xl">
          Security Integrated with the
          Guest Journey
        </h2>
       <div className="hidden md:flex">
         <h2 className="max-w-3xl mx-auto mb-6 text-3xl leading-tight md:text-5xl">
          Security Integrated with the <br />
          Guest Journey
        </h2>
       </div>

        {/* DESCRIPTION */}
        <div className="max-w-2xl mx-auto mb-12 space-y-4 text-sm leading-relaxed text-gray-400 md:text-base">
          <p>
            One of the defining advantages of working with VVIP Luxury is our ability to integrate security services seamlessly with our chauffeured transportation and concierge operations.
          </p>

          <p>
            This allows our clients to benefit from a unified service environment, where transportation, security, and concierge arrangements operate in harmony—reducing complexity while ensuring the highest level of coordination.
          </p>
        </div>

        {/* CARDS */}
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          
          {features.map((item, index) => (
            <div
              key={index}
              className="relative p-6 text-center border rounded-2xl md:p-8 bg-white/5 backdrop-blur-xl border-white/10"
            >
              {/* ICON CIRCLE */}
              <div className="w-14 h-14 mx-auto mb-5 flex items-center justify-center rounded-full bg-[#D4AF37]/10 border border-[#D4AF37]/30">
                <Image src={item.icon} alt="icon" width={22} height={22} />
              </div>

              {/* TITLE */}
              <h3 className="mb-2 text-base md:text-lg">
                {item.title}
              </h3>

              {/* DESC */}
              <p className="text-sm text-gray-400">
                {item.desc}
              </p>

              {/* SUBTLE GLOW */}
              <div className="absolute inset-0 border pointer-events-none rounded-2xl border-white/5" />
            </div>
          ))}
        </div>

      </section>
    </div>
  );
}